...
[![Base](https://img.shields.io/badge/Base-Mainnet-0052FF?style=flat-square)](https://base.org)
[![Next.js](https://img.shields.io/badge/Next.js-15-black?style=flat-square)](https://nextjs.org)
[![Farcaster](https://img.shields.io/badge/Farcaster-Mini%20App-855DCD?style=flat-square)](https://miniapps.farcaster.xyz)
...
|---|---|
| `NEXT_PUBLIC_ONCHAINKIT_API_KEY` | Free key from [Coinbase Developer Platform](https://portal.cdp.coinbase.com/) → **Client API key** (not a secret trading key) |
| `NEXT_PUBLIC_FEE_WALLET` | Wallet that receives the 5% platform fee — ships as a placeholder burn address, **change before going live** |
| `NEXT_PUBLIC_DEFAULT_RECIPIENT` | Wallet tips are sent to — swap for a per-profile address in production |
| `NEXT_PUBLIC_URL` | Your deployed domain, no trailing slash |
| `FARCASTER_HEADER` / `FARCASTER_PAYLOAD` / `FARCASTER_SIGNATURE` | Server-only — generated by Warpcast, see step 4 |
...
2. Warpcast → **Settings → Developer → Domains** → enter your domain →
   **Generate account association**.
3. Copy the three returned values into `FARCASTER_HEADER`,
   `FARCASTER_PAYLOAD`, `FARCASTER_SIGNATURE`.
4. Redeploy, then verify at `https://your-domain/.well-known/farcaster.json`.
5. Paste your URL into Warpcast's **Mini App Developer Tools** to preview
   the live embed.
...
|---|---|---|
| Base Smart Wallet / Coinbase Wallet | `coinbaseWallet()` | `preference: "all"` — supports smart wallet and EOA |
| Farcaster wallet | `farcasterMiniApp()` | Auto-activates only inside a Farcaster client |
| MetaMask | `metaMask()` | Dedicated connector for reliable deep links |
| Rainbow & others | `injected()` | Catches any EIP-1193 provider in the browser |

---

## Roadmap

See [`GRANT_STRATEGY.md`](./GRANT_STRATEGY.md) for the full 3-month plan.
Short version:
...
BaseFarCaster's tipping primitive is also exposed to AI agents via a
dedicated MCP server in [`mcp-server/`](./mcp-server) — quote a fee split
or construct unsigned tip calldata programmatically. It never holds keys
or signs transactions itself; see [`mcp-server/README.md`](./mcp-server/README.md)
for the safety design and setup.
